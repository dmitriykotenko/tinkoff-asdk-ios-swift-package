...
- (void)viewDidLoad
{
    ...
    self.title = self.viewTitleHead.length > 0 ? self.viewTitleHead : [ASDKUI_Texts current].acq_new_card_label;
    ...
    [self.tableView registerClass:[ASDKEmptyTableViewCell class] forCellReuseIdentifier:NSStringFromClass([ASDKEmptyTableViewCell class])];
        
    [self.tableView registerClass:[ASDKPaymentFormHeaderCell class] forCellReuseIdentifier:NSStringFromClass([ASDKPaymentFormHeaderCell class])];
	
	self.keyboardHeight = 0;
	
	ASDKBarButtonItem *cancelButton = [[ASDKBarButtonItem alloc]
                                       initWithTitle:[ASDKUI_Texts current].acq_btn_cancel
...
- (ASDKEmailCell *)emailCell
{
	if (!_emailCell)
	{
		...
		[_emailCell.emailTextField setPlaceholder:[ASDKUI_Texts current].acq_email];
...
- (ASDKCardInputTableViewCell *)cardRequisitesCell
{
	if (!_cardRequisitesCell)
	{
		...
		[_cardRequisitesCell setPlaceholderText:[ASDKUI_Texts current].acq_title_card_number];
...
- (ASDKPayButtonCell *)paymentButtonCell
{
	if (!_paymentButtonCell)
	{
		...
		[_paymentButtonCell setButtonTitle:[ASDKUI_Texts current].acq_title_add_card];
	}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	switch ([self.tableViewDataSource[indexPath.row] integerValue])
	{
		...
		{
			ASDKPaymentFormHeaderCell *cellTitle = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ASDKPaymentFormHeaderCell class])];
		...
		case CellAttachButton:
		{
			...
			{
				cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ASDKEmptyTableViewCell class]) forIndexPath:indexPath];
		...
		default:
			cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ASDKEmptyTableViewCell class])];
			break;
	}
		...
		[self.acquiringSdk initAttachCardWithCheckType:self.cardCheckType customerKey:self.customerKey success:^(ASDKResponseAddCardInit *response) {
			...
			if ([requestKey length] > 0)
			{
				...
				if ([emailString length] > 0)
				{
                    ...
                    additionalData[[ASDKUI_Texts current].acq_email] = emailString;
